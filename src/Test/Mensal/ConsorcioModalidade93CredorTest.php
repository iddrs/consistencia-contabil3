...
class ConsorcioModalidade93CredorTest extends TestBase {
    
    public function __construct(int $remessa, array $entidades) {
        $testName = 'Modalidade de Aplicação 93 e credores de consórcios';
    ...
    public function run(): void {
        ...
        $sql = "SELECT SUM(VALOR_EMPENHO)::DECIMAL
                ...
                WHERE REMESSA = %d
                    AND ENTIDADE IN (%s)
                    AND ANO_EMPENHO = %d
                    AND RUBRICA LIKE '__93%%'
                    AND CREDOR NOT IN (912, 8283)";
